            ...
            if incident_id not in corrected_incidents_dict.keys():
                ...
                if not df_corrected_irradiance_event.empty:
                    ...
                    try:
                        ...
                        index_event_start_time = index_event_end_time = None
                        percentage_of_timestamp_start = percentage_of_timestamp_end = None
                    ...
                    if active_timestamps == 0:
                        ...
                        if site == component:
                            ...
                            energy_lost = (sum(
                                [row_el["Corrected Irradiance"] * budget_pr_site.loc[str(row_el['Timestamp'].date())[:-2] + "01"] for
                                 index_el, row_el in
                                 df_irradiance_event_activeperiods.iterrows()]) * capacity * granularity) / 1000
                            ...
                            energy_lost = (sum(
                                [row_el["Corrected Irradiance"] *
                                 budget_pr_site.loc[str(row_el['Timestamp'].date())[:-2] + "01"] for index_el, row_el in
                                 df_irradiance_event_activeperiods.iterrows()]) * capacity * granularity) / 1000
...
def activehours_energylost_tracker(df, df_all_irradiance, df_all_export, budget_pr, active_events: bool = False,
    ...
    if active_events:
        ...
        for index, row in df_to_update.iterrows():
            ...
            if not df_irradiance_event.empty:
                # Get percentages of first timestamp to account for rounding
                index_event_start_time = df_irradiance_event.index.values[0]
                ...
                df_irradiance_event_activeperiods["Corrected Irradiance"] = \
                    df_irradiance_event_activeperiods[actual_column] * df_irradiance_event_activeperiods[
                        "Percentage of timestamp"]
...
def get_curtailment_timestamps(df_curtailment_events_site, setpoint_column, site_max_active_power_setpoint):
    ...
    return setpoints,start_timestamps , end_timestamps
...
def create_curtailment_incidents(site, all_site_info, df_irradiance, df_power, incidents, active_power_setpoint_df):
    ...
    df_curtailment_events_site = active_power_setpoint_df.filter(regex=f"Timestamp|\\[{site}\\].*").dropna().reset_index(drop=True)
    setpoint_column = df_curtailment_events_site.columns[1]

    setpoints,start_timestamps , end_timestamps = get_curtailment_timestamps(df_curtailment_events_site,
                                                                             setpoint_column,
                                                                             site_max_active_power_setpoint)
...
def calculate_curtailment_losses(power_irradiance_site, np_incidents_site, budget_pr, site, nominal_power, power_column_corrected,
                                 irradiance_column):
    ...
    power_irradiance_site['Power Lost'] = [(row['Expected Power'] - row[power_column_corrected]) if
                                           (row['Expected Power'] - row[power_column_corrected]) > 0 else 0
                                           for index, row in power_irradiance_site.iterrows()]

    power_irradiance_site['Corrected Power Lost'] = [
        (row['Corrected Expected Power'] - row[power_column_corrected]) if (row['Corrected Expected Power'] -
                                                                            row[
                                                                      power_column_corrected]) > 0 else 0 for
        index, row in power_irradiance_site.iterrows()]

    # print(power_irradiance_site)
    return power_irradiance_site


def get_power_irradiance_site_curtailment(site, power_site, irradiance_site_curated, stime, etime, irradiance_threshold,
                                          mlf_dlf_info):
    ...
    power_column_corrected = power_column + " Corrected"
    ...
    if not mlf_dlf_info.empty:
        ...
        power_irradiance_site["MLF_DLF"] = [1] * len(power_irradiance_site)

    power_irradiance_site[power_column_corrected] = power_irradiance_site[power_column] * \
                                                    power_irradiance_site["MLF_DLF"]

    return power_irradiance_site, irradiance_column, power_column, power_column_corrected

def curtailment_classic(source_folder, geography, geography_folder, site_selection, period,
                        irradiance_threshold: int = 20):

    (df_irradiance, df_power, active_power_setpoint_df, component_data, tracker_data, fmeca_data,
     site_capacities, fleet_capacity,
     budget_irradiance, budget_pr, budget_export,
     all_site_info, incidents, dest_file,
     mlf_dlf_info) = data_acquisition.read_curtailment_dataframes(geography, geography_folder,
                                                                  site_selection, period, irradiance_threshold)
        ...
        if curtailment_inc_df.empty:
            ...
            for index, row in curtailment_inc_df.iterrows():
                ...
                if budget_pr_stime == 0 and budget_pr_etime == 0:
                    ...
                    (power_irradiance_site, irradiance_column, power_column,
                     power_column_corrected) = get_power_irradiance_site_curtailment(site, power_site,
                                                                                     irradiance_site_curated, stime,
                                                                                     etime, irradiance_threshold,
                                                                                     mlf_dlf_info)

                    print(power_irradiance_site[["MLF_DLF",power_column, power_column_corrected]].head(100))
                    ...
                    if power_irradiance_site[irradiance_column].sum() == 0:
                        ...
                        power_irradiance_site = calculate_curtailment_losses(power_irradiance_site, np_incidents_site,
                                                                             budget_pr, site, nominal_power,
                                                                             power_column_corrected, irradiance_column)

                        # print(power_irradiance_site)

                        curtailment_inc_df.loc[index, "Expected Energy Loss (kWh)"] = (
                                power_irradiance_site['Power Lost'].sum() / 60) \
                            if (power_irradiance_site['Power Lost'].sum() / 60) > 0 else 0

                        curtailment_inc_df.loc[index, "Corrected Expected Energy Loss (kWh)"] = (
                                power_irradiance_site['Corrected Power Lost'].sum() / 60) \
                        ...
                        curtailment_inc_df.loc[index, "Active Hours (h)"] = (
                                power_irradiance_site.loc[
...
def clipping_classic(source_folder, geography, geography_folder, site_selection, period,
    ...
    budget_irradiance, budget_pr, budget_export, all_site_info, incidents, dest_file, folder_img = \
        data_acquisition.read_clipping_dataframes(geography, geography_folder, site_selection,
...
def contractual_calculation(source_folder, geography, geopgraphy_folder, site_selection, date_cutoff,
                                              irradiance_threshold):

    # <editor-fold desc="Get data to use">
    df_all_irradiance, df_all_export, component_data, tracker_data, fmeca_data, site_capacities, fleet_capacity, \
    budget_irradiance, budget_pr, budget_export, all_site_info, exclusion_periods_df, incidents, dest_folder, \
    site_list_per_om, monthly_periods_by_site, full_period_by_site, site_selection = \
        data_acquisition.read_contractual_dataframes(geography,geopgraphy_folder,site_selection,date_cutoff,
                                                     irradiance_threshold)

    # </editor-fold>

    # <editor-fold desc="Create dataframe of production and irradiance">

    for site in site_selection:
        print(site)

        start_date = monthly_periods_by_site[site]["All"][0]
        end_date = monthly_periods_by_site[site]["All"][1]

        contract_type = exclusion_periods_df.loc[site, "Contractual Availability Type"]
        ...
        production_irr_df = contractual.prod_irr_dataframe_site(site, contract_type, df_export_site, df_irradiance_site,
                                                    component_data_site, start_date, end_date, all_site_info, budget_pr,
                                                    geopgraphy_folder)

        try:
            all_sites_top_prod[site] = production_irr_df
        except NameError:
            all_sites_top_prod = {site: production_irr_df}
    ...
    # <editor-fold desc="Month Breakdown">
    # Month breakdown
    ...
    for site in site_selection:
        ...
        production_irr_df = all_sites_top_prod[site]
        periods_dict = monthly_periods_by_site[site]

        component_data_site = component_data.loc[component_data["Site"] == site]

        irradiance_threshold = all_site_info.loc[site, "Contractual Irradiance Threshold"]
        contract_type = all_site_info.loc[site, "Contractual Availability Type"]
        print(contract_type)
        ...
        site_kpis_df, site_incidents_per_period = contractual.contractual_availability_kpis(site, component_data_site,
                                                                                            ...
                                                                                            incidents, production_irr_df,
                                                                                            periods_dict,contract_type,
                                                                                            irradiance_threshold,
                                                                                            curated_column, export_column)

        incidents_by_site[site] = site_incidents_per_period

        try:
            all_sites_kpis[site] = site_kpis_df.T.rename_axis(site)
        except NameError:
            all_sites_kpis = {site: site_kpis_df.T.rename_axis(site)}

    # </editor-fold>

    # <editor-fold desc="Create KPIs by O&M">

    kpis_by_om_dict = {}

    for om in site_list_per_om.keys():
        site_list = site_list_per_om[om]

        all_site_main_kpi_dict = {}

        for site in site_list:
            site_kpis = all_sites_kpis[site]

            site_kpis = site_kpis.rename_axis(["Contractual Period"])
            month_cy_data = list(monthly_periods_by_site[site].keys())
            month_cy_data[-1] = site + " All"

            site_kpis["Month of CY"] = month_cy_data

            site_kpis = site_kpis.set_index(["Month of CY"])

            # print(site_kpis)

            site_main_kpi = site_kpis.loc[site + " All", :]
            # print(site_main_kpi)
            # site_main_kpi = site_main_kpi.rename_axis(site, inplace = True)

            all_site_main_kpi_dict[site] = site_main_kpi

        all_site_main_kpi_df = pd.DataFrame.from_dict(all_site_main_kpi_dict).T
        kpis_by_om_dict[om] = all_site_main_kpi_df
    ...
    return all_sites_kpis, kpis_by_om_dict, incidents_by_site, site_selection